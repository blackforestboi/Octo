...
Hex has one recording hotkey. In double-tap lock mode, a quick second activation starts normal recording. In single-tap start mode, the first activation starts that same locked recording instead. Both modes use the same terminal gestures: a quick terminal activation completes normally, and holding the terminal activation automatically stops and requests refinement as soon as the hold threshold is reached; key-up is not required. Holding the second activation in double-tap mode for 0.75 seconds (or a larger configured minimum) activates Screen Aware immediately and captures screen context; key-up never activates Screen Aware. Releasing before that threshold cancels its pending activation. Screen-aware recordings always refine regardless of the terminal gesture.

When enabled, the separate refinement hotkey invokes the same post-hoc refinement path for an active transcription or the most recently completed transcript. It never starts or stops a recording.
...
For Codex, Octo asks the resolved Codex runtime to report its active home directory, then reads the `.codex-global-state.json` project-state file there. It never assumes a conventional Codex folder and never writes Codex state. Before every handoff, Octo supplies the planner with the current Codex projects. Octo uses an ephemeral, read-only planning run that only returns focused task packages. It writes those packages and their full input context to its local journal before creating one normal, visible Recent Chat per package and starting the agent turn against that exact thread ID. There is no visible coordinator task. Each task starts in its selected Codex project folder, so it receives that project’s normal files, instructions, configuration, skills, plugins, and MCP servers rather than an empty generated workspace. If task creation or a later turn fails, the journal retains the original package and its creation state rather than losing it.

For Claude, Octo asks once for a parent directory, creates and retains a security-scoped `Agent Handoffs` folder, and starts the coordinator and child sessions there using Claude Code's native background-session (`claude --bg`) capability. It checks Claude Code's Agent View capability and sign-in state before creating a coordinator; an unavailable or signed-out CLI shows setup-required and leaves no task behind. These sessions are visible and manageable in Claude Code Agent View (`claude agents`). Claude Desktop has no supported import or attach route for locally created Agent View sessions, so a launched Octo task is not expected to appear in the Desktop sidebar. Octo keeps the durable task in Recent Handoffs and the Handoffs settings tab; selecting a Claude handoff opens its native session in Terminal with `claude attach <session-id>`. This is the fallback when the requested Desktop visibility is unavailable.

The gesture is unavailable when the configured hotkey already includes Shift, because the two chords would be ambiguous. Octo discovers the current Codex project catalogue automatically and does not create or modify Codex sidebar projects.
