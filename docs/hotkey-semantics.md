...
Hex has one recording hotkey. In double-tap lock mode, a quick second activation starts normal recording, while releasing that second activation after holding it for 0.75 seconds (or a larger configured minimum) captures screen context and forces refinement. The hold duration uses the physical macOS key-event timestamps, so delayed app event delivery cannot turn a short press into Screen Aware. While a locked recording is active, a quick terminal activation completes normally. Holding the terminal activation automatically stops and requests refinement as soon as the hold threshold is reached; key-up is not required. Screen-aware recordings always refine regardless of the terminal gesture.
